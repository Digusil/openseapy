...
from openseapy.utils import Smoother, QuantileScaler
...
class SNADatasetOld(CoreDataset):
    def __init__(
            self,
            hdf_file,
            feature_length=11,
...
class SNADataset(CoreDataset):
    ...
    def scaler_corr(self, pp):
        x = pp/self.original_pp
        gamma = 0.5
        return x + 1 / np.polyval([1, 1 / gamma, 1 / gamma ** 2, 1 / gamma], x)
